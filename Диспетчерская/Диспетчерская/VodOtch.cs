...
{
    public partial class VodOtch : Form
    {
        public VodOtch()
        {
            InitializeComponent();
        ...
        private void VodOtch_Load(object sender, EventArgs e)
        {
            Lists("Водопровод", OsnovList);
            Lists("ВодОбращ", ObsledList);
            Lists("ВодоРабота", RabList);
        }
        ...
        private void Formirovka_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                customers = string.Format("SELECT [Номер],{0}  FROM [Водопровод] {1}  ORDER BY [Номер] ASC ", osnov(OsnovList).Remove(0, 1), Where(0));
                nomer = string.Format("SELECT [Номер] FROM [Водопровод] {0}", Where(0));
            }
            catch
            {
                customers = string.Format("SELECT [Номер],{0}  FROM [Водопровод] {1}  ORDER BY [Номер] ASC ", "*", Where(0));
                nomer = string.Format("SELECT [Номер] FROM [Водопровод] {0}",Where(0));
            }
            ...
            if (checkBox8.Checked)
            {
                try
                {
                    customer = string.Format("SELECT [№ заявки],{0}  FROM [ВодОбращ] {1}  ORDER BY [№ заявки] ASC, [№ обращения] ASC ", osnov(ObsledList).Remove(0, 1), Wheres(1, nomer));
                }
                catch
                {
                    customer = string.Format("SELECT [№ заявки],{0}  FROM [ВодОбращ] {1}  ORDER BY [№ заявки] ASC, [№ обращения] ASC ", "*", Wheres(1,nomer));
                }
            ...
            if (checkBox9.Checked)
            {
                try
                {
                    customera = string.Format("SELECT [№ заявки],{0}  FROM [ВодоРабота] {1} ORDER BY [№ заявки] ASC, [№ работы] ASC", osnov(RabList).Remove(0, 1), Wheres(2, nomer));
                }
                catch
                {
                    customera = string.Format("SELECT [№ заявки],{0}  FROM [ВодоРабота] {1} ORDER BY [№ заявки] ASC, [№ работы] ASC", "*", Wheres(2, nomer));
                }
            ...
            try
            {
                ...
                using (SQLiteDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            if (reader.GetString(1) == "№ заявки" || reader.GetString(1) == "Номер")
                            {
                        ...
                        else if (po == "4")
                        {
                            if (po == "4")
                            {
                                if (data == 0)
                                {
                                    ...
                                    string skript = string.Format("Select [№ заявки] From [ВодоРабота] where [Разрытие] LIKE '%{0}%'", Razrit.Text);
                        ...
                        else if (po == "5")
                        {
                            if (po == "5")
                            {
                                if (data == 0)
                                {
                                    ...
                                    string skript = string.Format("SELECT [№ заявки] FROM [ВодоРабота] WHERE [Вид повреждения] LIKE '%{0}%'", VidPovr.Text);
                                    SQLiteCommand command = new SQLiteCommand(skript, sql);
